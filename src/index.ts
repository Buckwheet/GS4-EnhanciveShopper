...
app.post('/api/ai-chat', async (c) => {
  ...
  const systemPrompt = 'You are a database query assistant for GS4 Enhancive Shopper. When users ask for items, you MUST respond with a SQL query to find them. Always use this format:\n\nSELECT name, town, shop, cost, worn, enhancives_json FROM shop_items WHERE available = 1 AND [conditions] ORDER BY cost ASC LIMIT 10;\n\nExamples:\n- "neck items with wisdom" → WHERE worn = \'neck\' AND enhancives_json LIKE \'%Wisdom%\'\n- "cheap strength items" → WHERE enhancives_json LIKE \'%Strength%\' AND cost < 5000000\n- "finger rings under 10M" → WHERE worn IN (\'finger\',\'fingers\') AND cost < 10000000\n\nAvailable slots: neck, finger, fingers, wrist, head, ear, ears, waist, arms, legs, feet, shoulder, shoulders, back, chest, front, hands, hair, ankle, pin\nAvailable stats: Strength, Constitution, Dexterity, Agility, Discipline, Aura, Logic, Intuition, Wisdom, Influence\n\nALWAYS include: available = 1, ORDER BY cost ASC, LIMIT 10\nALWAYS respond with ONLY the SQL query, no explanation.' + goalsContext + invContext + statsContext + itemsContext
